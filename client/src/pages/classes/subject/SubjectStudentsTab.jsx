...
import { faPlus, faUser, faSpinner, faSearch, faTrash } from '@fortawesome/free-solid-svg-icons';

const SubjectStudentsTab = ({ subjectClassId, gradelevelClassId }) => {
  const { token } = useAuth();
  ...
  const fetchStudents = async () => {
    ...
    setError('');
    try {
      const response = await axios.get(`${BASE_URL}/classes/subject-enrollments`, {
  ...
  // Enroll student in subject class
  const handleAddStudent = async (student) => {
    setAddLoadingId(student.RegNumber);
    setAddError('');
    try {
      const response = await axios.post(`${BASE_URL}/classes/subject-enrollments`, {
        student_regnumber: student.RegNumber,
        subject_class_id: subjectClassId,
        gradelevel_class_id: gradelevelClassId
      }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (response.data.success) {
        ...
        setAddError(response.data.message || 'Failed to enroll student.');
      }
    } catch (err) {
  ...
  const handleDeEnroll = async (enrollment) => {
    ...
    setDeEnrollError('');
    try {
      await axios.delete(`${BASE_URL}/classes/subject-enrollments/${enrollment.id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchStudents();
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div className="text-xs text-gray-500">No students enrolled in this subject class.</div>
      ) : (
                    ...
                    {showConfirmId === student.id && (
                      <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
                        <div className="bg-white p-8 rounded shadow w-full max-w-md">
                          ...
                          {deEnrollError && <div className="text-xs text-red-600 mb-2">{deEnrollError}</div>}
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded shadow w-full max-w-2xl relative">
            {/* Loading overlay for enrollment */}
            {addLoadingId && (
              <div className="absolute inset-0 bg-white bg-opacity-70 flex items-center justify-center z-50">
                <FontAwesomeIcon icon={faSpinner} spin className="h-8 w-8 text-gray-700" />
                <span className="ml-3 text-gray-700 text-sm">Enrolling...</span>
              </div>
            )}
            <h3 className="text-base font-medium text-gray-900 mb-4">Enroll Student in Subject Class</h3>
            <form onSubmit={handleSearch} className="flex mb-4">
              ...
              <button
                ...
                disabled={searchLoading || !!addLoadingId}
              >
