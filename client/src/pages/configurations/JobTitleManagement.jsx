...
const JobTitleManagement = () => {
  ...
  const fetchJobTitles = async () => {
    try {
      ...
      const response = await axios.get(`${BASE_URL}/configurations/job-titles`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = modalMode === 'create' 
        ? `${BASE_URL}/configurations/job-titles`
        : `${BASE_URL}/configurations/job-titles/${selectedJobTitle.id}`;
  ...
  const confirmDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`${BASE_URL}/configurations/job-titles/${selectedJobTitle.id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  return (
    <div>
              ...
              jobTitles.map((jobTitle) => (
                <tr key={jobTitle.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap text-xs">
                    <div className="flex items-center">
                      <FontAwesomeIcon icon={faUserTie} className="h-4 w-4 text-gray-400 mr-3" />
                      <div>
      ...
      {totalPages > 1 && (
        <div className="mt-4 flex items-center justify-between">
          <div className="text-xs text-gray-500">
            Showing {((currentPage - 1) * limit) + 1} to {Math.min(currentPage * limit, totalRecords)} of {totalRecords} job titles
          </div>
            ...
            <div className="mt-3">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-medium text-gray-900">
                  {modalMode === 'create' ? 'Add New Job Title' : 'Edit Job Title'}
                </h3>
                <button
      ...
      {showDeleteModal && selectedJobTitle && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-96 bg-white">
            <div className="mt-3 text-center">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">Delete Job Title</h3>
              <p className="text-xs text-gray-500 mb-4">
                Are you sure you want to delete "{selectedJobTitle.title}"? This action cannot be undone.
              </p>
